...
public class CompanyAdminService : ICompanyAdminService
{
    ...
    public async Task<CompanyAdminDto> CreateAsync(CreateCompanyAdminRequest request, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<CompanyAdminDto> UpdateAsync(UpdateCompanyAdminByIdRequest idRequest, UpdateCompanyAdminRequest request, CancellationToken cancellationToken = default)
    {
        // Decrypt ID using AutoMapper
    ...
    public async Task<bool> DeleteAsync(DeleteCompanyAdminRequest request, CancellationToken cancellationToken = default)
    {
        ...
        // End all sessions
        await _sessionRepo.EndAllSessionsAsync(adminId, SessionEndReason.AccountDeactivated, "Admin account deleted", cancellationToken);
    ...
    public async Task<bool> ResetPasswordAsync(ResetPasswordByIdRequest idRequest, ResetAdminPasswordRequest request, CancellationToken cancellationToken = default)
    {
        // Decrypt ID using AutoMapper
        var adminId = _mapper.Map<Guid>(idRequest);
        var admin = await _adminRepo.GetByIdAsync(adminId, null, cancellationToken);
        ...
        await _sessionRepo.EndAllSessionsAsync(adminId, SessionEndReason.PasswordChanged, "Password reset by admin", cancellationToken);
    ...
    public async Task<bool> UnlockAccountAsync(UnlockAccountByIdRequest request, CancellationToken cancellationToken = default)
    {
        // Decrypt ID using AutoMapper
        var adminId = _mapper.Map<Guid>(request);
        var admin = await _adminRepo.GetByIdAsync(adminId, null, cancellationToken);
    ...
    public async Task<int> TerminateAllSessionsAsync(TerminateSessionsByIdRequest idRequest, string? reason = null, CancellationToken cancellationToken = default)
    {
        // Decrypt ID using AutoMapper
        var adminId = _mapper.Map<Guid>(idRequest);
        var activeSessions = await _sessionRepo.GetActiveSessionsByAdminAsync(adminId, cancellationToken);
        var count = activeSessions.Count;

        await _sessionRepo.EndAllSessionsAsync(adminId, SessionEndReason.AdminTerminated, reason, cancellationToken);
        await _adminRepo.ClearCurrentSessionAsync(adminId, cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);

        _logger.LogInformation("Terminated {Count} sessions for company admin {AdminId}", count, adminId);
        return count;
    }
    ...
    public async Task<AdminLoginResponse> LoginAsync(AdminLoginRequest request, string? ipAddress = null, CancellationToken cancellationToken = default)
    {
        ...
        var existingSessions = await _sessionRepo.GetActiveSessionsByAdminAsync(admin.Id, cancellationToken);
        
        if (admin.SessionPolicy == AdminSessionPolicy.SingleSession && existingSessions.Any())
        {
            // Terminate existing sessions
            await _sessionRepo.EndAllSessionsAsync(admin.Id, SessionEndReason.NewSessionStarted, "New login", cancellationToken);
        }
        ...
        await _adminRepo.UpdateCurrentSessionAsync(admin.Id, sessionId, request.DeviceHash, request.DeviceName, ipAddress, cancellationToken);
        
        await _unitOfWork.SaveChangesAsync(cancellationToken);

        _logger.LogInformation("Company admin {Username} logged in from {IP}", admin.Username, ipAddress);

        response.Success = true;
        response.Message = _localizer["CompanyAdmin.LoginSuccess"];
        response.Token = sessionId; // In production, generate a JWT with this session ID
        response.SessionId = sessionId;
        response.ExpiresAtUtc = expiresAt;
    ...
    public async Task<bool> LogoutAsync(string sessionId, CancellationToken cancellationToken = default)
    {
        ...
        await _sessionRepo.EndSessionAsync(sessionId, SessionEndReason.Logout, null, cancellationToken);
    ...
    public async Task<CompanyAdminDetailsDto?> ValidateSessionAsync(string sessionId, CancellationToken cancellationToken = default)
    {
        var session = await _sessionRepo.GetBySessionIdAsync(sessionId, cancellationToken);
        ...
        {
            await _sessionRepo.EndSessionAsync(sessionId, SessionEndReason.Inactivity, null, cancellationToken);
            await _unitOfWork.SaveChangesAsync(cancellationToken);
            return null;
        }
    ...
    public async Task<bool> ChangePasswordAsync(Guid adminId, CompanyAdminChangePasswordRequest request, CancellationToken cancellationToken = default)
    {
        var admin = await _adminRepo.GetByIdAsync(adminId, null, cancellationToken);
        if (admin == null)
            throw new NotFoundException(_localizer["CompanyAdmin.NotFound"]);
    ...
    public async Task<bool> UpdateSessionSettingsAsync(Guid adminId, UpdateSessionSettingsRequest request, CancellationToken cancellationToken = default)
    {
        var admin = await _adminRepo.GetByIdAsync(adminId, null, cancellationToken);
        if (admin == null)
            throw new NotFoundException(_localizer["CompanyAdmin.NotFound"]);
    ...
    public async Task<List<CompanyAdminSessionDto>> GetSessionHistoryAsync(Guid adminId, int limit = 50, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<List<CompanyAdminSessionDto>> GetActiveSessionsAsync(Guid adminId, CancellationToken cancellationToken = default)
    {
        var sessions = await _sessionRepo.GetActiveSessionsByAdminAsync(adminId, cancellationToken);
        return sessions.Select(MapToSessionDto).ToList();
    }

    public async Task<bool> TerminateSessionAsync(Guid adminId, string sessionId, CancellationToken cancellationToken = default)
    {
        var session = await _sessionRepo.GetBySessionIdAsync(sessionId, cancellationToken);
        if (session == null || session.AdminId != adminId) return false;

        await _sessionRepo.EndSessionAsync(sessionId, SessionEndReason.AdminTerminated, "Terminated by user", cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);
    ...
    public async Task<bool> IsUsernameAvailableAsync(string username, Guid? excludeAdminId = null, CancellationToken cancellationToken = default)
    {
        return !await _adminRepo.UsernameExistsAsync(username, excludeAdminId, cancellationToken);
    }

    public async Task<bool> CompanyHasAdminAsync(CompanyHasAdminRequest request, CancellationToken cancellationToken = default)
    {
        // Debug: Log encrypted and decrypted IDs
        _logger.LogInformation("CompanyHasAdminAsync - Encrypted CompanyId: {EncryptedId}", request.CompanyId);
        
        // Decrypt CompanyId using AutoMapper
        var companyId = _mapper.Map<Guid>(request);
        
        _logger.LogInformation("CompanyHasAdminAsync - Decrypted CompanyId: {DecryptedId}", companyId);
